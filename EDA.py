...
def perform_eda(df, sample_size=10000):
    ...
    if len(class_pd) >= 2:
        ...
        if len(fraud_row) > 0:
            ...
            fraud_pct = (
                fraud_row['count'].values[0] / total
            ) * 100
    ...
    sample_fraction = (
        min(1.0, sample_size / total_rows)
        ...
        if len(pdf_clean) > 0:
            ...
            if "is_fraud" in corr_matrix.columns:
                ...
                for feature in fraud_corr_sorted.head(5).index:
                    ...
                    print(
                        f"{feature}: "
                        f"{fraud_corr[feature]:.3f}"
                    )

    # =====================================================
    # Summary Statistics by Fraud
    # =====================================================
    ...
    if "is_fraud" in pdf.columns and len(pdf) > 0:
        ...
        if "user_avg_amt" in pdf.columns:
            agg_dict["user_avg_amt"] = [
                "mean",
                "median"
            ]

        if "user_transaction_count" in pdf.columns:
            agg_dict["user_transaction_count"] = [
                "mean",
                "median"
            ]

        if "trans_count_last_24h" in pdf.columns:
            agg_dict["trans_count_last_24h"] = [
                "mean",
                "median"
            ]
    ...
    if "is_night" in pdf.columns:
        ...
        night_fraud = (
            pdf[pdf["is_night"] == 1]["is_fraud"]
            .mean() * 100
        )

        day_fraud = (
            pdf[pdf["is_night"] == 0]["is_fraud"]
            .mean() * 100
        )
    ...
    if "amt" in pdf.columns:
        ...
        high_amt_fraud = (
            pdf[pdf["amt"] > amt_median]["is_fraud"]
            .mean() * 100
        )
    ...
    if "user_transaction_count" in pdf.columns:
        ...
        high_freq_fraud = (
            pdf[pdf["user_transaction_count"] > freq_median]["is_fraud"]
            .mean() * 100
        )
    ...
    if "is_rapid_transaction" in pdf.columns:
        ...
        rapid_fraud = (
            pdf[pdf["is_rapid_transaction"] == 1]["is_fraud"]
            .mean() * 100
        )

        normal_fraud = (
            pdf[pdf["is_rapid_transaction"] == 0]["is_fraud"]
            .mean() * 100
        )
